...
export function VagasRecomendadas ({ vagasRecomendadas }: vagasRecomendadasProps) {
  if (!vagasRecomendadas || vagasRecomendadas.length === 0) {
    return (
      <div className="p-6 rounded-lg">
        <h2 className="text-lg font-bold text-white text-center mb-6">Vagas Recomendadas</h2>
        <div className="text-center py-8">
          ...
          <p className="text-slate-400 text-lg font-medium mb-2">Nenhuma vaga disponível</p>
      ...
      <div className="mt-4 space-y-4 p-4 rounded-lg">
        {vagasRecomendadas.map(vaga => (
          <div key={vaga.id} className="p-4 border bg-slate-800 border-slate-700 rounded-lg ">
            ...
            <button>
              <Link href={`/aluno/vagas/${vaga.id}`} className="text-sm font-semibold text-blue-500 hover:underline cursor-pointer">Ver detalhes</Link>
            </button>
          </div>
        ))}
      </div>
